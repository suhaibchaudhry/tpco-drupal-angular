...
/**
 * @file
 * Contains \TxprintcoMigration.
 */

abstract class TxprintcoMigrateBase extends Migration {
  ...
  public function __construct($arguments) {
    ...
    if (!in_array($this->entityType, array('node', 'taxonomy_term'))) {
      throw new Exception('\TxprintcoMigration supports only nodes and terms.');
    }
  ...
  protected function getMigrateDirectory() {
    return variable_get('txprintco_migrate_directory', FALSE) ? variable_get('txprintco_migrate_directory') : drupal_get_path('module', 'txprintco_migrate');
  }
}
