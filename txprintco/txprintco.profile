 ...
 */
function txprintco_form_install_configure_form_alter(&$form, $form_state) {
 ...
 */
function txprintco_install_tasks() {
  $tasks = array();

  $tasks['txprintco_setup_variables'] = array(
    'display_name' => st('Set Variables'),
    'display' => FALSE,
  );

  $tasks['txprintco_setup_og_permissions'] = array(
    'display_name' => st('Set Blocks'),
    'display' => FALSE,
  );

  // Run this as the last task!
  $tasks['txprintco_setup_rebuild_permissions'] = array(
 ...
 */
function txprintco_setup_variables() {
  $variables = array(
    // Features default export path.
    'features_default_export_path' => 'profiles/txprintco/modules/custom',
    // Mime-mail.
    'mimemail_format' => 'full_html',
    'mimemail_sitestyle' => FALSE,
    'mimemail_name' => 'Txprintco',
    'mimemail_mail' => 'info@txprintco.com',
    // jQuery versions.
 ...
 * created.
 */
function txprintco_setup_og_permissions() {
 ...
 */
function txprintco_setup_blocks() {
 ...
 * installation.
 */
function txprintco_setup_rebuild_permissions() {
  node_access_rebuild();
}
